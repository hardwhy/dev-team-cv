...
import { slugify, generateStoragePath } from '@dev-team-cv/shared-utils';
...
export function ProjectsPage() {
  ...
  const openCreate = () => {
    setEditing(null); setForm(EMPTY); setSelectedMembers([]);
    setThumbFile(null); setSaveError(null); setModalOpen(true);
  };
  ...
  const openEdit = (p: Project) => {
    ...
    setSelectedMembers(p.team_members?.map((m) => m.id) ?? []);
    setThumbFile(null); setSaveError(null); setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (thumbFile) {
        // Use file extension from mime type for safety, include content-type to avoid CORS issues
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
          {projects.map((p) => (
            <Card key={p.id} className="flex flex-col">
              {/* Thumbnail */}
              <div className="w-full h-36 rounded-lg bg-[var(--surface-overlay)] overflow-hidden mb-3 shrink-0">
                {p.thumbnail
              ...
              <div className="flex flex-wrap gap-1 mb-3 flex-1">
                {p.technologies.slice(0, 4).map((t) => <Badge key={t} label={t} />)}
                {p.technologies.length > 4 && <span className="text-xs text-[var(--text-muted)] self-center">+{p.technologies.length - 4}</span>}
              </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Project' : 'Add Project'} size="xl">
        <div className="grid md:grid-cols-2 gap-5">
          ...
          {/* Row 4: Project Type + Featured — both with labels so they align */}
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              {/* Custom chevron with proper spacing */}
          ...
          <div className="md:col-span-2">
            <ThumbnailPicker
              ...
              onFileChange={(file) => {
                setThumbFile(file);
                // If cleared, also clear the stored URL
